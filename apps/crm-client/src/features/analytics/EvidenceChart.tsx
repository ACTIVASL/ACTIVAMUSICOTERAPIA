...
export const EvidenceChart: React.FC<EvidenceChartProps> = ({ sessions }) => {
    ...
    const data = normalizedData.map((s) => {
        ...
        return {
            ...
            fullDate: s.dateObj.toLocaleDateString('es-ES', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }),
    ...
    if (data.length === 0) {
        return (
            <div className="h-64 flex flex-col items-center justify-center bg-slate-50 border border-slate-200 border-dashed rounded-2xl p-6 text-center">
                ...
                <p className="text-xs text-slate-400 mt-1">Registre sesiones con valoración para ver la curva.</p>
            </div>
        );
    }
            ...
            <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data} margin={{ top: 20, right: 10, left: -20, bottom: 0 }}>
                    ...
                    <YAxis
                        ...
                        fontWeight={600}
                    />
                    <Tooltip content={<CustomTooltip />} cursor={{ stroke: '#EC4899', strokeWidth: 2, strokeDasharray: '4 4' }} />
                    <ReferenceLine y={1.5} stroke="#CBD5E1" strokeDasharray="3 3" label={{ value: 'Objetivo', position: 'insideRight', fill: '#94A3B8', fontSize: 10 }} />
                    <Area
                        type="monotone"
                        dataKey="score"
