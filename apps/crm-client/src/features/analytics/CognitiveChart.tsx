    ...
    const data = history
        .map(h => {
            ...
            return {
                ...
                moca,
                mmse,
                fullDate: dateObj.toLocaleDateString('es-ES', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })
            };
        })
        ...
        if (active && payload && payload.length) {
            return (
                <div className="bg-white/95 backdrop-blur-md p-4 rounded-xl shadow-xl border border-pink-100">
                    <p className="text-xs font-bold text-slate-400 mb-2 uppercase tracking-wider">{payload[0].payload.date}</p>
                    {payload.map((entry) => (
                        <div key={entry.name} className="flex items-center gap-2 mb-1">
                            <span className={`w-2 h-2 rounded-full ${entry.name === 'MOCA' ? 'bg-pink-500' : 'bg-indigo-500'}`} />
        ...
        <div className="w-full h-full min-h-[300px] font-sans relative">
            <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data} margin={{ top: 20, right: 10, left: -20, bottom: 0 }}>
                    ...
                    {/* Clinical Cutoffs */}
                    <ReferenceLine y={26} stroke="#10b981" strokeDasharray="3 3" label={{ value: 'Normal', position: 'insideRight', fill: '#10b981', fontSize: 10 }} />
