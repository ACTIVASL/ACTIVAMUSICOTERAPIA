...
import {
    UserPlus, CalendarCheck, FileSignature, CreditCard, ShieldCheck,
    FileX, Loader2, FileText, CheckCircle2, ArrowUpRight, Download
} from 'lucide-react';
...
export const TitaniumActivityFeed: React.FC<TitaniumActivityFeedProps> = ({ activities, isLoading }) => {
    ...
    const getIcon = (type: string, _metadata?: ActivityLogItem['metadata']) => {
        ...
        switch (type) {
            case 'patient':
                return <div className={`${baseClass} bg-emerald-50 border-emerald-100 text-emerald-600`}><UserPlus size={16} /></div>;
            case 'session':
                return <div className={`${baseClass} bg-pink-50 border-pink-100 text-pink-600`}><CalendarCheck size={16} /></div>;
            case 'report':
                return <div className={`${baseClass} bg-purple-50 border-purple-100 text-purple-600`}><FileSignature size={16} /></div>;
            case 'finance':
                return <div className={`${baseClass} bg-amber-50 border-amber-100 text-amber-600`}><CreditCard size={16} /></div>;
            case 'security':
                return <div className={`${baseClass} bg-indigo-50 border-indigo-100 text-indigo-600`}><ShieldCheck size={16} /></div>;
            case 'delete':
                return <div className={`${baseClass} bg-red-50 border-red-100 text-red-500`}><FileX size={16} /></div>;
            default:
                return <div className={`${baseClass} bg-slate-50 border-slate-100 text-slate-600`}><CheckCircle2 size={16} /></div>;
        }
    };
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            link.setAttribute('download', `registro_actividad_gsheets_${format(new Date(), 'yyyyMMdd_HHmm')}.csv`);
                                                ...
                                                {item.type === 'finance' && item.metadata && 'amount' in item.metadata && (
                                                    <div className="mt-1 flex items-center gap-2">
                                                        <span className="text-xs font-mono bg-emerald-50 text-emerald-700 px-1.5 rounded border border-emerald-100">
                                                            {new Intl.NumberFormat('es-ES', { style: 'currency', currency: 'EUR' }).format(item.metadata.amount || 0)}
                                                        </span>
                                                        {item.metadata.status === 'paid' && (
                                                            <span className="text-[10px] bg-slate-100 text-slate-500 px-1 rounded">Pagado</span>
                                                        )}
                                                    </div>
                                                )}
