...
import { AlertTriangle, Calendar, Clock, HeartPulse, History, PauseCircle, PlayCircle, ShieldCheck, TrendingUp } from 'lucide-react';
...
export const LifecycleTab: React.FC<LifecycleTabProps> = ({ patient, activeSessions, onUpdate }) => {
                ...
                <div className="flex flex-col md:flex-row gap-8 items-center relative z-10">
                    ...
                    <div className="relative w-32 h-32 flex items-center justify-center">
                        <svg className="w-full h-full -rotate-90" viewBox="0 0 36 36">
                            ...
                            <path
                                d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"
                                fill="none"
                                stroke={healthScore.value > 80 ? '#10b981' : healthScore.value > 50 ? '#f59e0b' : '#ef4444'}
                                strokeWidth="3"
                    ...
                    <div className="flex-1">
                        ...
                        <p className="text-slate-500 mb-4 max-w-lg">
                            El Health Score indica la viabilidad clínica y administrativa. Mantén este índice alto asegurando asistencia regular y pagos al día.
                        </p>
                        <div className="flex gap-2 flex-wrap">
                            {healthScore.deductions.map((d, i) => (
                                <span key={i} className="px-2 py-1 bg-red-50 text-red-600 text-xs font-bold rounded-lg border border-red-100">
                                    -{d.points} {d.reason}
                                </span>
                            ))}
                ...
                <div className="bg-white rounded-2xl border border-slate-200 p-6 flex flex-col justify-between">
                    <div>
                        ...
                        <div className="flex items-center gap-3 text-sm text-slate-600 bg-slate-50 p-3 rounded-lg border border-slate-100">
                            ...
                            <span>
                                Fin de ciclo estimado: <strong>{format(cycleMetrics.estimatedEndDate, "d 'de' MMMM", { locale: es })}</strong>
                            </span>
                        </div>
                    </div>

                    <div className="mt-8 flex gap-3">
                        <Button variant="secondary" className="flex-1" icon={isPaused ? PlayCircle : PauseCircle} onClick={handleTogglePause}>
                            {isPaused ? 'Reanudar' : 'Pausar Tratamiento'}
                        </Button>
                        <Button variant="primary" className="flex-1" icon={History} onClick={() => alert('Próximamente: Histórico de Ciclos')}>
                            Ver Historial
                        </Button>
                    </div>
                </div>
                    ...
                    <div className="relative pl-4 border-l-2 border-slate-200 space-y-8 ml-2">
                        ...
                        <div className="relative">
                            ...
                            <p className="text-xs text-slate-500 mt-1">
                                {patient.createdAt ? format(new Date(patient.createdAt), "PPP", { locale: es }) : 'Fecha desconocida'}
                            </p>
                        </div>

                        {/* Current Status */}
                        <div className="relative">
                            <div className={`absolute -left-[21px] top-1 w-4 h-4 rounded-full border-2 border-white animate-pulse ${isPaused ? 'bg-amber-400' : 'bg-indigo-500'}`} />
