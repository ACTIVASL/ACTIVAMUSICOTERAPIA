...
export const EvaluationTab: React.FC<EvaluationTabProps> = ({ patient, onOpenCognitive, onUpdate, clinicSettings, showToast }) => {
    ...
    const generateAndUploadPDF = async () => {
        ...
        try {
            ...
            const printHeader = () => {
                ...
                doc.text(clinicSettings.name || 'Clínica Método Activa', pageWidth - 20, 15, { align: 'right' });
                doc.text(`${clinicSettings.address || ''} | ${clinicSettings.phone || ''}`, pageWidth - 20, 20, { align: 'right' });
            };
            ...
            if (!isChild) {
                ...
                doc.setFontSize(24); doc.setTextColor(236, 0, 140);
                ...
                doc.setFontSize(24); doc.setTextColor(50);
            ...
            const drawRow = (label: string, value: number, y: number) => {
                ...
                if (value === 1) { text = "BAJO"; color = [234, 179, 8]; } // Yellow
                if (value === 2) { text = "MEDIO"; color = [59, 130, 246]; } // Blue
                if (value === 3) { text = "ALTO"; color = [16, 185, 129]; } // Green
            ...
            if (!isChild) {
                ADULT_DEV_DOMAINS.forEach(domain => {
                    ...
                    domain.items.forEach(item => {
                        ...
                        const val = (idx !== -1 && functionalScores[idx] !== undefined) ? functionalScores[idx] : 0;
            ...
            // CHILD LOOP
                ...
                // Iterate categories
                [...CHILD_DEV_DOMAINS, { id: 'musical', title: 'Perfil Musical', items: CHILD_MUSICAL_PROFILE.flatMap(p => p.items) }].forEach(domain => {
                    doc.setFont('helvetica', 'bold');
                    doc.setTextColor(30);
                    doc.text(domain.title.toUpperCase(), 20, currentY);
                    currentY += 8;
                    ...
                    domain.items.forEach(item => {
                        ...
                        if (domain.id === 'musical') {
                            // Complex find
                            const catId = CHILD_MUSICAL_PROFILE.find((c) => c.items.includes(item))?.id || 'rhythm';
            ...
            doc.text("Documento generado por Método Activa Clinical OS", centerX, pageHeight - 10, { align: 'center' });
            ...
            <Card className="p-8">
                ...
                <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 grid grid-cols-2 md:grid-cols-4 gap-4 items-center">
                    {patient.age >= 18 && (
                        <>
                            <div>
                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">MOCA</span>
                                <span className="text-lg font-bold text-slate-800">{patient.cognitiveScores?.moca || '-'}</span>
                            </div>
                            <div>
                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">MMSE</span>
                                <span className="text-lg font-bold text-slate-800">{patient.cognitiveScores?.mmse || '-'}</span>
                            </div>
                            <div>
                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">GDS</span>
                                <span className="text-lg font-bold text-slate-800">{patient.cognitiveScores?.gds || '-'}</span>
                            </div>
                        </>
                    )}
                    <div>
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Fecha</span>
                        <span className="text-sm font-medium text-slate-600">{patient.cognitiveScores?.date || '-'}</span>
                    </div>
                </div>
                                ...
                                {patient.evaluationHistory.map((record) => (
                                    <tr key={record.id} className="bg-white hover:bg-slate-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-slate-600">
                                            <span className="bg-pink-100 text-pink-700 px-2 py-0.5 rounded text-xs font-bold mr-2">MOCA: {record.results.moca}</span>
                                            <span className="bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded text-xs font-bold mr-2">MMSE: {record.results.mmse}</span>
                                            <span className="bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded text-xs font-bold">GDS: {record.results.gds}</span>
                                        </td>
