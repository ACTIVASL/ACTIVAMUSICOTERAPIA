...
import { UploadCloud, FileText, Trash2, Eye, FileImage, File, Loader2, ShieldCheck, Receipt } from 'lucide-react';
...
const DocumentCard = ({ doc, sectionColor, getIcon, onDelete, formatSize }: DocumentCardProps) => {
    ...
    return (
        <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm flex items-center gap-3 group hover:border-brand-300 transition-all hover:shadow-md relative overflow-hidden">
            <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${sectionColor.split(' ')[0]} ${sectionColor.split(' ')[2]}`}>
                {getIcon(doc.type, doc.category)}
            </div>

            <div className="flex-1 min-w-0 mr-16">
                <a href={doc.url} target="_blank" rel="noopener noreferrer" className="font-bold text-slate-800 text-sm truncate hover:text-brand-600 block leading-tight">
                    {doc.name}
                </a>
...
export const DocumentsTab: React.FC = () => {
    ...
    const [selectedCategory, setSelectedCategory] = useState<keyof typeof DocumentCategoryEnum.enum>('report');
    ...
    const sections = [
        { id: 'consent', label: 'Documentación Legal', color: 'bg-blue-50 border-blue-100 text-blue-800' },
        { id: 'invoice', label: 'Facturación', color: 'bg-emerald-50 border-emerald-100 text-emerald-800' },
        { id: 'report', label: 'Historial Clínico', color: 'bg-indigo-50 border-indigo-100 text-indigo-800' },
        { id: 'lab', label: 'Pruebas Externas', color: 'bg-purple-50 border-purple-100 text-purple-800' },
        { id: 'general', label: 'Archivo General', color: 'bg-slate-50 border-slate-100 text-slate-800' },
        ...
        <div className="space-y-8 animate-in fade-in pb-10">
            ...
            >
                <div className="max-w-md mx-auto space-y-4">
                    <div className="flex justify-center">
                        <div className={`w-14 h-14 rounded-full flex items-center justify-center transition-colors ${isDragging ? 'bg-white text-brand-600' : 'bg-slate-50 text-slate-400'}`}>
                    ...
                    <div className="flex items-center justify-center gap-2 bg-slate-50 p-1.5 rounded-lg border border-slate-200 inline-flex w-full">
                        ...
                        <select
                            ...
                            onChange={(e) => setSelectedCategory(e.target.value as keyof typeof DocumentCategoryEnum.enum)}
            ...
            <div className="space-y-6">
                {sections.map(section => {
                    ...
                    if (docs.length === 0 && !['general', 'consent', 'invoice'].includes(section.id)) return null;

                    return (
                        <div key={section.id} className="animate-in slide-in-from-bottom-2 duration-500">
                            <h4 className={`font-bold text-xs uppercase tracking-wider mb-3 flex items-center gap-2 ${section.color.split(' ')[2]}`}>
                                {section.label} <span className="bg-white px-1.5 py-0.5 rounded-full shadow-sm text-[10px] border">{docs.length}</span>
                            </h4>
