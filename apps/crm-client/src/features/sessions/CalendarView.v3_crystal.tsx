...
import { ChevronLeft, ChevronRight, CalendarCheck, Users, Sparkles, Clock, Sun } from 'lucide-react';
import { Patient, GroupSession, NavigationPayload } from '../../lib/types';
import { Toast } from '@monorepo/ui-system';
import { format, addMonths, isSameDay, startOfMonth, endOfMonth, eachDayOfInterval } from 'date-fns';
      ...
      <div className="flex-1 flex flex-col bg-white/80 backdrop-blur-2xl border border-white/60 shadow-2xl shadow-indigo-100/50 rounded-3xl overflow-hidden relative">
        ...
        <div className="flex items-center justify-between p-6 border-b border-indigo-50/50">
          <div className="flex items-center gap-4">
            <div className="flex items-center bg-white rounded-full p-1.5 shadow-sm border border-indigo-50">
              <button onClick={() => handleMonthChange(-1)} className="p-2 hover:bg-slate-50 rounded-full transition-colors text-slate-400 hover:text-indigo-600">
                <ChevronLeft size={20} />
              </button>
            ...
            <div className="hidden xl:flex items-center gap-4 text-xs font-bold text-slate-400 bg-white/50 px-4 py-2 rounded-full border border-white/60 shadow-sm">
              <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-pink-400 shadow-[0_0_8px_rgba(244,114,182,0.4)]"></div>Individual</div>
              <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-indigo-400 shadow-[0_0_8px_rgba(129,140,248,0.4)]"></div>Grupal</div>
            </div>
          </div>

          <div className="flex items-center gap-3">
            <Button size="sm" onClick={() => onOpenQuickAppointment('existing')}
            ...
            <Button size="sm" onClick={() => onOpenGroupModal('schedule')}
              variant="secondary"
        ...
        <div className="grid grid-cols-7 border-b border-indigo-50/30 bg-white/40">
          {['Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb', 'Dom'].map(d => (
            <div key={d} className="py-4 text-center text-[11px] font-bold uppercase text-slate-400 tracking-widest">
              {d}
            </div>
          ))}
        </div>
        ...
        <div className="flex-1 grid grid-cols-7 grid-rows-5 lg:grid-rows-6 p-6 gap-3 overflow-y-auto custom-scrollbar">
          {/* Padding Days */}
          {Array(calendarDays.padding).fill(null).map((_, i) => (
            ...
            return (
              <div key={i}
                ...
                <div className="flex justify-between items-start mb-2">
                  <span className={`text-sm font-bold w-8 h-8 flex items-center justify-center rounded-full transition-all ${isSelected ? 'bg-white/20 text-white' :
                    isToday ? 'bg-pink-50 text-pink-600 shadow-sm' : 'text-slate-600 group-hover:text-indigo-600'
                    }`}>
                    {format(day, 'd')}
                  </span>
                  {hasEvents && (
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${isSelected ? 'bg-white text-indigo-600' : 'bg-indigo-50 text-indigo-600'
                      }`}>
                      {dayEvents.length}
                    </span>
                  )}
                ...
                <div className="flex-1 flex flex-col justify-end gap-1.5">
                  {dayEvents.slice(0, 3).map((ev, idx) => (
                    <div key={idx} className="flex items-center gap-1.5">
                      <div className={`w-1.5 h-1.5 rounded-full shrink-0 shadow-sm ${ev.type === 'group' ? 'bg-indigo-400' : ev.status === 'absent' ? 'bg-red-400' : 'bg-pink-400'
                        } ${isSelected ? 'ring-1 ring-white/50' : ''}`}></div>
                      <span className={`text-[9px] font-semibold truncate ${isSelected ? 'text-indigo-50' : 'text-slate-500'}`}>
                        {ev.time} {ev.title?.split(' ')[0]}
                      </span>
                    </div>
                  ))}
                  {dayEvents.length > 3 && (
                    <div className={`text-[9px] font-bold text-center mt-1 ${isSelected ? 'text-white/70' : 'text-slate-300'}`}>
                      +{dayEvents.length - 3}
                    </div>
                  )}
                </div>
      ...
      <div className="w-full lg:w-[400px] shrink-0 flex flex-col gap-6 h-full relative z-20">
        ...
        <div className="bg-white/90 backdrop-blur-xl p-8 rounded-3xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.05)] border border-white flex items-center justify-between relative overflow-hidden group">
          <div className="relative z-10">
            <h2 className="text-5xl font-black text-transparent bg-clip-text bg-gradient-to-br from-slate-800 to-slate-600 tracking-tighter">{format(selectedDay, 'd')}</h2>
            <p className="text-sm font-bold text-indigo-400 uppercase tracking-widest mt-1">{format(selectedDay, 'MMMM, EEEE', { locale: es })}</p>
          </div>
        ...
        <div className="flex-1 bg-white/50 backdrop-blur-md rounded-3xl border border-white/50 flex flex-col p-1 relative">
          ...
          {selectedEvents.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center opacity-50">
              ...
              <p className="text-sm text-slate-400 mt-2 max-w-[200px]">No hay sesiones programadas para este día.</p>
            </div>
          ) : (
                ...
                <div key={i}
                  onClick={() => {
                    if (ev.type === 'individual' && ev.patientId) {
                      ...
                      onOpenGroupModal('evolution', { ...groupSessions.find(g => g.id === ev.id), date: format(ev.dateObj, 'yyyy-MM-dd') } as GroupSession);
                    }
                  }}
                  ...
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex items-center gap-3">
                      <div className={`px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-2 shadow-sm ${ev.type === 'group' ? 'bg-indigo-50 text-indigo-600' : 'bg-pink-50 text-pink-600'}`}>
