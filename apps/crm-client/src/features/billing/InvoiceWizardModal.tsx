...
export const InvoiceWizardModal: React.FC<InvoiceWizardModalProps> = ({ isOpen, onClose }) => {
    ...
    const [billingData, setBillingData] = useState<NonNullable<ClinicSettings['billing']>>({
        legalName: '', nif: '', address: '', logoUrl: '', email: '', phone: ''
    });
    ...
    const availableGroups = useMemo(() => {
        const uniqueNames = new Set(allGroupSessions.map(g => g.groupName).filter((name): name is string => !!name));
        ...
        queryFn: async () => {
            if (mode === 'INDIVIDUAL' && selectedPatientId) {
                ...
                return sessions
                    ...
                    .map(s => ({ ...s, patientId: selectedPatientId, _type: 'individual' as const } as unknown as BillableSession));
            }
            if (mode === 'GROUP' && selectedGroupName) {
                const sessions = await BillingRepository.getUnbilledGroupSessions(selectedGroupName);
                return sessions.map(s => ({ ...s, _type: 'group' as const } as unknown as BillableSession));
            }
            return [];
        },
    ...
    const selectedSessions = billableSessions.filter(s => selectedSessionIds.includes(String(s.id)));
    ...
    const handleGenerate = async () => {
        ...
        // Upsert Settings
        try { await updateSettings({ billing: billingData }); } catch (e) { console.error("Settings autosave failed", e); }
        ...
        const invoicePayload = {
            ...
            items: selectedSessions.map(s => ({
                id: crypto.randomUUID(),
                description: mode === 'INDIVIDUAL' ? `Sesión Individual - ${s.date}` : `Sesión Grupal (${selectedGroupName}) - ${s.date}`,
                quantity: 1, unitPrice: Number(s.price) || 0, total: Number(s.price) || 0, sessionId: s.id as string
            })),
            subtotal: total, taxRate: 0, taxAmount: 0, total: total,
            notes: mode === 'INDIVIDUAL' ? `Factura música para ${selectedPatient?.name}` : `Factura de servicios grupales: ${selectedGroupName}`
        };

        const batchSessions = selectedSessions.map(s => ({
            id: String(s.id), type: s._type, patientId: s.patientId, price: Number(s.price), date: String(s.date)
        }));

        try {
            await createInvoice({ invoice: invoicePayload, sessions: batchSessions });
            onClose();
            setStep(1); setSelectedPatientId(null); setSelectedGroupName(null); setSelectedSessionIds([]);
        } catch (e) {
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl overflow-hidden flex flex-col max-h-[95vh]">
                ...
                <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 font-medium">Asistente de Facturación Inteligente (Refactored)</p>
                    </div>
                    <button onClick={onClose} className="p-2 hover:bg-slate-200 rounded-full text-slate-400"><X size={20} /></button>
                </div>

                <div className="flex-1 overflow-y-auto p-6">
                    {step === 1 && <WizardStep1_Mode onSelectMode={(m) => { setMode(m); setStep(2); }} />}

                    {step === 2 && <InvoiceClientSelector
                        mode={mode} patients={patients} availableGroups={availableGroups}
                        onSelectPatient={(id) => { setSelectedPatientId(id); setStep(3); }}
                        onSelectGroup={(name) => { setSelectedGroupName(name); setStep(3); }}
                        onChangeMode={() => setStep(1)}
                    />}

                    {step === 3 && <InvoiceSessionSelector
                        mode={mode} billableSessions={billableSessions} selectedSessionIds={selectedSessionIds}
                        onSelectionChange={setSelectedSessionIds} onChangeSelection={() => setStep(2)}
                    />}

                    {step === 4 && <WizardStep4_Summary
                        mode={mode} selectedSessions={selectedSessions}
                        selectedName={mode === 'INDIVIDUAL' ? selectedPatient?.name || '' : selectedGroupName || ''}
                        invoiceDate={invoiceDate} setInvoiceDate={setInvoiceDate}
                        customInvoiceNumber={customInvoiceNumber} setCustomInvoiceNumber={setCustomInvoiceNumber}
                        billingData={billingData} setBillingData={setBillingData}
                    />}
                </div>
                ...
                <div className="p-6 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
                    {step > 1 && (
                        <button onClick={() => setStep(prev => (prev - 1) as WizardStep)} className="px-6 py-3 rounded-xl font-bold text-slate-500 hover:bg-slate-200 transition-colors">
                            Atrás
                        </button>
                    )}
                    {step < 4 ? (
                        <button
                            ...
                            disabled={(step === 2 && ((mode === 'INDIVIDUAL' && !selectedPatientId) || (mode === 'GROUP' && !selectedGroupName))) || (step === 3 && selectedSessionIds.length === 0)}
                        ...
                        <button
                            onClick={handleGenerate} disabled={isCreating}
