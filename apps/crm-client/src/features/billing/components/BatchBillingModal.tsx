...
export const BatchBillingModal: React.FC<BatchBillingModalProps> = ({ isOpen, onClose }) => {
    ...
    useEffect(() => {
        ...
        const newCandidates: BillingCandidate[] = patients
            .map((patient: Patient) => {
                ...
                const total = unbilledSessions.reduce((acc: number, s: Session) => acc + (Number(s.price) || 0), 0);
    ...
    const toggleCandidate = (patientId: string) => {
        setCandidates(prev => prev.map(c =>
            c.patient.id === patientId ? { ...c, selected: !c.selected } : c
        ));
    };
            ...
            try {
                ...
                const invoicePayload = {
                    ...
                    items: sessions.map(s => ({
                        id: crypto.randomUUID(),
                        description: `Sesión de Musicoterapia - ${s.date}`,
                        quantity: 1,
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/80 backdrop-blur-sm p-4 animate-in fade-in duration-300">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                    ...
                    {!isGenerating && <button onClick={onClose} className="p-2 hover:bg-slate-200 rounded-full text-slate-400"><X size={20} /></button>}
                </div>
                ...
                <div className="flex-1 overflow-hidden flex flex-col md:flex-row">
                    ...
                    <div className="w-full md:w-1/3 bg-slate-50 p-6 border-r border-slate-100 flex flex-col gap-6">
                        {/* Month Selector */}
                        <div>
                            <label className="text-xs font-black text-slate-400 uppercase tracking-widest mb-2 block">Periodo de Facturación</label>
                            <input
                                type="month"
                        ...
                        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-3">
                            <div className="flex justify-between items-center">
                                ...
                                <span className="font-bold text-slate-900 text-lg">{candidates.filter(c => c.selected).length}</span>
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="text-sm text-slate-500">Sesiones</span>
                                <span className="font-bold text-slate-900 text-lg">
                                    {candidates.filter(c => c.selected).reduce((acc, c) => acc + c.sessions.length, 0)}
                                </span>
                            </div>
                            <div className="pt-3 border-t border-slate-100 flex justify-between items-center">
                                ...
                                <span className="font-black text-indigo-600 text-2xl">
                                    {candidates.filter(c => c.selected).reduce((acc, c) => acc + c.total, 0).toFixed(2)}€
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="mt-auto bg-amber-50 p-4 rounded-xl border border-amber-100 text-xs text-amber-700 flex gap-2">
                            ...
                            <p>Se generarán facturas en estado <b>PENDIENTE</b>. Revisa los datos antes de enviarlas a los clientes.</p>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 flex flex-col overflow-hidden bg-white relative">
                        ...
                        {isGenerating && (
                            <div className="absolute inset-0 z-10 bg-white/90 backdrop-blur-sm flex flex-col items-center justify-center p-8">
                                <div className="w-full max-w-sm space-y-4 text-center">
                                    <h3 className="text-2xl font-black text-slate-900 animate-pulse">Generando Facturas...</h3>
                                    ...
                                    {progress === 100 && (
                                        <Button onClick={onClose} className="w-full mt-4" variant="primary">Finalizar</Button>
                                    )}
                                </div>
                            </div>
                        )}
                        ...
                        <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => setCandidates(prev => prev.map(c => ({ ...c, selected: true })))} className="text-xs font-bold text-indigo-600 hover:text-indigo-800">Todos</button>
                                <span className="text-slate-300">|</span>
                                <button onClick={() => setCandidates(prev => prev.map(c => ({ ...c, selected: false })))} className="text-xs font-bold text-slate-500 hover:text-slate-700">Ninguno</button>
                            </div>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
                            {isLoading ? (
                                ...
                                <div className="flex flex-col items-center justify-center h-full text-slate-400">
                                    ...
                                    <p>No hay sesiones pendientes en este periodo.</p>
                                </div>
                            ) : (
                                candidates.map((candidate) => (
                                    ...
                                    >
                                        ...
                                        <div className="flex-1">
                                            <h4 className={`font-bold text-sm ${candidate.selected ? 'text-indigo-900' : 'text-slate-600'}`}>
                                                {candidate.patient.name}
                                            </h4>
                                            <p className="text-xs text-slate-400">
                                                {candidate.sessions.length} sesiones encontradas
                                            </p>
                                        </div>

                                        <div className="text-right">
                                            <span className={`block font-black ${candidate.selected ? 'text-indigo-600' : 'text-slate-400'}`}>
                        ...
                        <div className="p-4 border-t border-slate-100 bg-white">
                            <Button
                                ...
                                icon={Rocket}
                            >
                                Generar Facturas ({candidates.filter(c => c.selected).reduce((acc, c) => acc + c.total, 0).toFixed(2)}€)
                            </Button>
                        </div>
                    </div>
                </div>
