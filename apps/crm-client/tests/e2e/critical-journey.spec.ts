    ...
    test('Complete Clinical Loop: Login -> Create Patient -> Quick Appointment', async ({ page }) => {
        ...
        try {
            ...
            if (url.includes('dashboard')) {
                ...
                const demoButton = page.locator('button').filter({ hasText: /Acceso Demo/i }).first();
            ...
            const isVisible = await page.getByRole('button', { name: /Guardar/i }).first().isVisible();
            console.log(`Save button visible? ${isVisible}`);

            // Strategy A: Click visible button
            const visibleSave = page.locator('button').filter({ hasText: /Guardar/i }).filter({ hasNotText: /Cancelar/i });
