...
export const LeadCaptureModal = ({ initialInterest }: { initialInterest?: string }) => {
    ...
    const [formData, setFormData] = useState({ name: '', email: '', phone: '', interest: initialInterest || 'general' });
        ...
        try {
            ...
            const response = await fetch("https://formsubmit.co/ajax/3f2f544e1402d95130017c1f71354bdc", {
                ...
                body: JSON.stringify({
                    ...
                    _template: "box",
                    _captcha: "false",
                    _autoresponse: "¡Gracias por contactar con Activa Musicoterapia! Hemos recibido tu solicitud.",
                    // Spanish Data Labels
    ...
    if (step === 'success') {
        return (
            <div className="text-center py-12 px-6 animate-fade-in font-['Inter']">
                ...
                <h3 className="text-2xl font-bold text-slate-900 mb-3 font-['Outfit']">¡Gracias por registrarte!</h3>
                <p className="text-slate-600 mb-8 max-w-sm mx-auto leading-relaxed">
                    Hemos recibido tus datos correctamente. En breves momentos nos pondremos en contacto contigo para resolver tus dudas.
                </p>
    ...
    return (
        <div className="p-0 font-['Inter']">
            ...
            <div className="bg-slate-50 p-8 border-b border-gray-100 text-center">
                ...
                <h3 className="text-3xl font-bold text-slate-900 font-['Outfit'] mb-3 leading-tight">Tu Espacio Terapéutico</h3>
                <p className="text-slate-500 text-sm leading-relaxed max-w-sm mx-auto">
                    Facilítanos tu contacto para enviarte nuestra <strong>propuesta clínica</strong> y <strong>disponibilidad de horarios</strong>. Sin compromiso.
                </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-5">
                ...
                <div className="space-y-4">
                    <div className="relative group">
                        <User className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-[#EC008C] transition-colors" size={20} />
                        <input
                            required
                            type="text"
                    ...
                    <div className="relative group">
                        <Mail className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors duration-300 ${formData.email.includes('@') ? 'text-[#EC008C]' : 'text-slate-400 group-focus-within:text-[#EC008C]'}`} size={20} />
                        <input
                            required
                            type="email"
                        ...
                        <div className={`absolute right-4 top-1/2 -translate-y-1/2 transition-all duration-500 ${formData.email.includes('@') && formData.email.length > 5 ? 'opacity-100 scale-100' : 'opacity-0 scale-50'}`}>
                            <CheckCircle2 className="text-green-500" size={20} />
                        </div>
                    </div>

                    <div className="relative group">
                        <Phone className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-[#EC008C] transition-colors" size={20} />
                        <input
                            type="tel"
                    ...
                    <div className="relative group">
                        <Activity className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-[#EC008C] transition-colors" size={20} />
                        <select
                ...
                <div className="pt-2">
                    ...
                    >
                        {isSubmitting ? 'ENVIANDO...' : 'SOLICITAR INFORMACIÓN'} <Send size={20} className={isSubmitting ? 'animate-pulse' : ''} />
                    </button>
