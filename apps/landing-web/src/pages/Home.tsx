...
export const Home = () => {
    ...
    const modalConfig: Record<string, { title: string; icon: JSX.Element; content: JSX.Element; className?: string; hideTitle?: boolean }> = {
        book: {
        ...
        <>
            ...
            {activeModal && modalConfig[activeModal] && (
                <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 modal-overlay font-sans">
                    <div className="absolute inset-0 bg-brand-dark/80 backdrop-blur-xl transition-opacity" onClick={closeModal}></div>
                    <div className="relative bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden modal-content border border-white/20 ring-1 ring-black/5 transform transition-all scale-100 animate-fade-in-up">
                        <div className={`bg-white border-b border-gray-100 flex justify-between items-center z-20 relative sticky top-0 ${modalConfig[activeModal].hideTitle ? 'p-4 border-none absolute right-0 bg-transparent' : 'p-8'}`}>
                            {!modalConfig[activeModal].hideTitle && (
                                <div className="flex items-center gap-5">
                                    ...
                                    <h3 className="font-display font-bold text-2xl text-brand-dark leading-none tracking-tight">{modalConfig[activeModal].title}</h3>
                                </div>
                            )}
                            <button onClick={closeModal} className={`text-gray-400 hover:text-brand-dark transition-colors p-2.5 rounded-full hover:bg-gray-50 ${modalConfig[activeModal].hideTitle ? 'bg-white/80 shadow-sm' : ''}`}>
                                <X size={24} />
                            </button>
                        </div>
                        <div className={`${modalConfig[activeModal].className || 'p-8'} max-h-[80vh] overflow-y-auto`}>
